...
int main(int argc, char* argv[]){
    ...
    try {
        ...
        for (int j=0; j < 10000; ++j) {
            ...
            // This is a test of setWriteCursor() more than anything else.
            loader::BufferUdp msgBuf((char*)(msgOut.data()), msgOut.length());
            msgBuf.setWriteCursor(msgOut.length());
            socket.send_to(boost::asio::buffer(msgBuf.begin(), msgBuf.getCurrentWriteLength()), receiverEndpoint);
